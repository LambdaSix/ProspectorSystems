...
namespace ProspectorSystems.Data.Script
{
    ...
    public class MyOreDetectorComponent
    {
        ...
        public Dictionary<MyVoxelBase, MyOreDepositGroup> DepositGroupsByEntity { get; } = new Dictionary<MyVoxelBase, MyOreDepositGroup>();
        public ConcurrentCachingHashSet<MiningInformation> MiningInformation { get; } = new ConcurrentCachingHashSet<MiningInformation>();

        // Aggregated list is for PB and terminal display
        public ConcurrentCachingList<MiningInformationPB> AggregatedMiningInformation { get; } = new ConcurrentCachingList<MiningInformationPB>();
        private readonly ConcurrentDictionary<string, MiningInformationPB> m_aggregateCache = new ConcurrentDictionary<string, MiningInformationPB>();

        public float Complete
        {
            get
            {
                if (DepositGroupsByEntity.Count == 0)
                    return 0;
                return DepositGroupsByEntity.Values.Average(g => g.Complete);
            }
        }
        ...
        public bool Scanning
        {
            get
            {
                if (DepositGroupsByEntity.Count == 0 || AggregatedMiningInformation.Count == 0)
                    return true;
                return DepositGroupsByEntity.Values.Any((g) => g.Scanning);
            }
        }
        ...
        private void BuildAggregatedResult()
        {
            ...
            //lock (m_miningInformationLock)
            //{
                foreach (var deposit in MiningInformation)
                {
            ...
            //{
                AggregatedMiningInformation.ClearList();

                foreach (var info in m_aggregateCache.Values)
                    AggregatedMiningInformation.Add(info);

                AggregatedMiningInformation.ApplyChanges();

                Logger.Instance.LogDebugOnGameThread($"Aggregate count: {AggregatedMiningInformation.Count}");
            //}
        }
